...
plotQualityProfile(fnFs[1:6])# I don't know what is negative but the curve is weird
...
annotation <- c(annotation, 100 - sum(is.na(taxa[,1]))/dim(taxa)[1]*100) # 10% of data has no associated kingdom
annotation <- c(annotation, 100 - sum(is.na(taxa[,2]))/dim(taxa)[1]*100) # 57% at the phylum lvl
annotation <- c(annotation, 100 - sum(is.na(taxa[,3]))/dim(taxa)[1]*100)
annotation <- c(annotation, 100 - sum(is.na(taxa[,4]))/dim(taxa)[1]*100)
annotation <- c(annotation, 100 - sum(is.na(taxa[,5]))/dim(taxa)[1]*100) # 78% at the family lvl and around the same for the genus lvl
annotation <- c(annotation, 100 - sum(is.na(taxa[,6]))/dim(taxa)[1]*100)
annotation
barplot(annotation, ylim = c(0,100))
```

```{r }
library(ggplot2)
library(forcats)
df=data.frame(val=annotation, id=colnames(taxa))
pdf("../img/annotation_rate.pdf", width = 4, height = 3)
ggplot(df, aes(x=fct_reorder(id,-val), y=val)) + geom_col(color="black", fill="gray") +
    theme_classic() + ylim(0,100) + ggtitle("Annotation rate of the DUPE dataset") + 
  ylab("% of annotated sequences") + xlab("taxonomic rank")
dev.off()
```

# extract the non identified sequences

```{r}
non_id <- taxa[is.na(taxa[,6]),]
write.table(non_id, '../data/non_id.txt') # To save the table
hist(nchar(row.names(non_id))) # to check the distribution of the length of the sequences
```

# Separate the already identified sequences to merge later on

```{r}
identified <- taxa[!is.na(taxa[,6]),]
write.table(identified, '../data/identified.txt') # To save the table
# jpeg(file="../img/annotation_rate.jpeg")
hist(nchar(row.names(identified))) # to check the distribution of the length of the sequences
# dev.off()
```

# create a training at FASTA format
